...
using System.Text;
public partial class cp_ucCosonuoitrong : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            {
                lnkAdd.NavigateUrl = "~/cp/Default.aspx?page=CosonuoitrongUpdate_&amp;do=add";
            }
            else if (Convert.ToInt64(Session["groupID"].ToString()) == 4)
            {
                lnkAdd.NavigateUrl = "~/cp/Tochucchungnhan/Default.aspx?page=CosonuoitrongUpdate_&amp;do=add&ctr=adm";
                hypImport.NavigateUrl = "~/cp/Tochucchungnhan/Default.aspx?page=ImportCSNT";
            }
        }
    }
    private void napGridView()
    {
        ...
        if (iGroupID != 1)
        {
            ...
            {
                Response.Write("<script>alert('Tổ chức này không quản lý bất cứ Cơ sở Trồng trọt nào.');</script>");
                return;
            }
    ...
    protected void grvNews_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        if(Convert.ToInt64(Session["groupID"].ToString())==1)
            Response.Redirect("~/cp/Default.aspx?page=CosonuoitrongUpdate_");
        else if(Convert.ToInt64(Session["groupID"].ToString())==4)
            Response.Redirect("~/cp/Tochucchungnhan/Default.aspx?page=CosonuoitrongUpdate_&ctr=adm");
    }
    ...
    protected void grvNews_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Convert.ToInt64(Session["groupID"].ToString()) == 1)
            {
                hypXuly.NavigateUrl = "~/cp/default.aspx?page=Xulyvipham&cosonuoitrongID="+PK_iCosonuoitrongID;
                hypCapgiayphep.NavigateUrl = "~/cp/default.aspx?page=GiayphepVietGap&PK_iCosonuoitrongID="+PK_iCosonuoitrongID;
            }
            else if (Convert.ToInt64(Session["groupID"].ToString()) == 4)
            {
                hypXuly.NavigateUrl = "~/cp/Tochucchungnhan/default.aspx?page=Xulyvipham&ctr=adm&cosonuoitrongID=" + PK_iCosonuoitrongID;
                hypCapgiayphep.NavigateUrl = "~/cp/Tochucchungnhan/default.aspx?page=GiayphepVietGap&ctr=adm&PK_iCosonuoitrongID=" + PK_iCosonuoitrongID;
            }
        }
