...
public partial class cp_ucCosonuoitrongUpdate : System.Web.UI.UserControl
{
        ...
        if (!Page.IsPostBack)
        {
           ...
           // LoadMap(getAdd());
            ...
            if (lstCosonuoitrong.Count>0)
            {
                ...
                // vì 1 cơ sơ trồng có thể có nhiều Username
    ...
    private void taoCSNT_giadinh()
    {
        try
        {
            ...
            //Thêm Cơ sở Trồng trọt giả định
    ...
    //{
    //    string body = "<b>Tài khoản</b>: ##UserName## đã được kích hoạt thành công. Bạn có thể sử dụng tài khoản này để đăng nhập và nhập các thông tin về Cơ sở Trồng trọt.";
    ...
    private void EditCosonuoitrong()
    {
        ...
        try
        {
            ...
            if (btnDKThongtinCoSoNuoi.CommandName == "Edit")
            {
                ...
                {
                    Response.Write("<script>alert('Đăng ký Cơ sở Trồng trọt thành công!');location='Default.aspx';</script>");
                }
                else
                {
                    Response.Write("<script>alert('Cập nhập thành công!');location='Default.aspx?page=Cosonuoitrong'</script>");
